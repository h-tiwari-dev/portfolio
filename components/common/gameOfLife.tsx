...
export default function GameOfLife() {
  ...
  // Config
  const CELL_SIZE = 5;
  const ALIVE_COLOR = '#ff3366';
  const GLOW_COLOR = 'rgba(255, 51, 102, 0.4)';
  ...
  useEffect(() => {
    const init = () => {
      ...
      if (
        !universeRef.current ||
        universeRef.current.width !== cols ||
        universeRef.current.height !== rows
      ) {
  ...
  const handleInteraction = (
    ...
    if (col >= 0 && col < universe.width && row >= 0 && row < universe.height) {
      ...
      if (
        lastPos.current?.col === col &&
        lastPos.current?.row === row &&
        !forceDraw
      )
        return;
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full min-h-[200px] md:min-h-[150px] relative overflow-hidden bg-slate-950/20 group/sim touch-pan-y"
    >
      <canvas
        ref={canvasRef}
      ...
      <div className="absolute top-6 right-6 z-50 flex items-center space-x-2 opacity-0 group-hover/sim:opacity-100 transition-opacity duration-300">
        <button
          ...
          className="p-1.5 rounded-md bg-slate-900/80 border border-white/10 text-slate-400 hover:text-rose-400 hover:border-rose-400/50 transition-all"
      ...
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-5 group-hover/sim:opacity-20 transition-opacity">
        <span className="text-[12px] font-mono uppercase tracking-[1em] text-rose-400 font-bold">
      ...
      <div className="absolute bottom-2 left-2 z-20 pointer-events-none opacity-0 group-hover/sim:opacity-40 transition-opacity flex items-center space-x-2">
        <MousePointer2 size={10} className="text-rose-400" />
