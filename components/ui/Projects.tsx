        ...
        <div className="h-full w-full p-5 overflow-hidden flex flex-col group">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-[10px] font-bold text-cyan-400 uppercase tracking-[0.2em] flex items-center">
                    <Activity size={14} className="mr-2 text-cyan-600" />
                    Process_Queue.exe
                </h3>
            ...
            <div className="space-y-3 overflow-y-auto scrollbar-none pr-1">
                {projects.map((project, index) => (
                    <div
                        key={index}
                        className="bg-neutral-900 p-3 border border-neutral-800 hover:border-cyan-900 hover:bg-neutral-800 transition-all group/item cursor-pointer"
                    >
                        ...
                        <div className="flex flex-wrap gap-1">
                            {project.tags.map((tag, i) => (
                                <span key={i} className="text-[8px] font-mono bg-slate-950 text-cyan-600 px-1.5 py-0.5 border border-cyan-900">
                                    {tag}
                                </span>
                            ))}
                        </div>
