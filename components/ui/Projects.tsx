    ...
    return (
        <div className="h-full w-full p-5 overflow-hidden flex flex-col group">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-[10px] font-bold text-sky-400 uppercase tracking-[0.2em] flex items-center">
                    <Activity size={14} className="mr-2 text-sky-500/70" />
                    Process_Queue.exe
                </h3>
            ...
            <div className="space-y-3 overflow-y-auto scrollbar-none pr-1">
                {projects.map((project, index) => (
                    <div
                        key={index}
                        className="bg-white/[0.02] rounded p-3 border border-white/5 hover:border-sky-500/20 hover:bg-white/[0.04] transition-all group/item cursor-pointer"
                    >
                        <div className="flex items-center justify-between mb-1.5">
                            <h4 className="text-[12px] font-bold text-slate-200 group-hover/item:text-sky-300 transition-colors uppercase font-mono tracking-tight">{project.title}</h4>
                        ...
                        <div className="flex flex-wrap gap-1">
                            {project.tags.map((tag, i) => (
                                <span key={i} className="text-[8px] font-mono bg-slate-950 text-sky-500/70 px-1.5 py-0.5 rounded border border-sky-500/10">
                                    {tag}
                                </span>
                            ))}
                        </div>
