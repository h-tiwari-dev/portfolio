    ...
    return (
        <div className="h-full w-full p-5 overflow-hidden flex flex-col group">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-[12px] font-bold text-indigo-400 uppercase tracking-[0.2em] flex items-center">
                    <Cpu size={14} className="mr-2 text-indigo-500/70" />
                    Core_Stack.kernel
                </h3>
            </div>
                ...
                {categories.map((cat, idx) => (
                    <div key={idx} className="space-y-2.5 overflow-hidden">
                        <div className="text-[12px] font-mono text-slate-400 uppercase tracking-tighter flex items-center mb-1">
                            <div className="w-1.5 h-1.5 bg-indigo-500/50 rounded-full mr-2"></div>
                            {cat.name}
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                            {cat.skills.map((skill, i) => (
                                <span
                                    key={i}
                                    className="bg-indigo-500/10 text-slate-200 text-[11px] font-mono px-2 py-1 rounded border border-indigo-500/20 hover:border-indigo-400/40 hover:bg-indigo-500/20 hover:text-white transition-all cursor-default"
                                >
                                    {skill}
                                </span>
                            ))}
