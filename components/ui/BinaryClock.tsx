    ...
    <div className="flex flex-col items-center">
      <div className="flex space-x-0.5 mb-1">
        {bits.map((bit, i) => (
          <div
            key={i}
            className={`w-1 h-1 rounded-sm transition-all duration-500 ${
              bit === '1'
                ? 'bg-rose-400 shadow-[0_0_5px_#ff3366]'
                : 'bg-white/10'
            }`}
          />
        ))}
      </div>
      <span className="text-[6px] font-mono text-slate-500 font-bold">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div className="flex items-center space-x-2 sm:space-x-3 px-2 sm:px-3 py-1 sm:py-1.5 bg-white/[0.03] border border-white/5 rounded-md">
      <div className="flex flex-col mr-1 sm:mr-2 pr-1 sm:pr-2 border-r border-white/5">
        <span className="text-[7px] font-mono text-rose-500/50 font-bold uppercase tracking-widest">
          Sys_Clock
        </span>
